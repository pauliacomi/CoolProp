def collect(tmp, name):
    print('copying sources')
    shutil.copytree(
        os.path.join('..', '..', '..', 'src'), os.path.join(tmp, 'src')
    )
...
if __name__ == '__main__':
    ...
    subprocess.check_call(
        ...
        shell=True,
        cwd=os.path.join('..', '..', '..', 'dev'),
        stdout=sys.stdout,
        stderr=sys.stderr
    )
    subprocess.check_call(
        'python generate_constants_module.py',
        shell=True,
        cwd='..',
        stdout=sys.stdout,
        stderr=sys.stderr
    ...
    for pyx in ['CoolProp.pyx', '_constants.pyx']:
        subprocess.check_call(
            'cython --cplus ' + os.path.split(pyx)[1],
            shell=True,
            cwd=os.path.join('..', 'CoolProp'),
            stdout=sys.stdout,
            stderr=sys.stderr
        )
    ...
    try:
        collect(tmp, 'CoolProp')

        # Make the source distro in this folder
        subprocess.check_call(
            ' '.join(['python', 'setup.py', 'install'] + sys.argv[1::]),
            shell=True,
            cwd=tmp,
            stdout=sys.stdout,
            stderr=sys.stderr
